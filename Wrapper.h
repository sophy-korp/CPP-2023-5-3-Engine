...
template<typename classType, typename...Args>
Wrapper::Wrapper(classType* instance, int(classType::* Func)(Args...), std::map<std::string, int> args) {
	if (!instance || !Func) {
		throw std::exception();
	}
	for (auto&& arg : args) {
...
template<typename classType, typename funcType, size_t...index>
int Wrapper::indexBinding(classType instance, funcType Func, std::vector<int> args,
	std::index_sequence<index...>) {
